<x-layouts.app title="Category Master - Admin Panel Web">
    <div x-data="{ 
        showModal: false, 
        editMode: false,
        ...
        openEditModal(cat) {
            ...
            this.actionUrl = '{{ url('categories') }}/' + cat.id;
            this.catId = cat.id;
            this.name = cat.name;
            this.displayOrder = cat.display_order || 1;
            this.status = Boolean(cat.status);
            this.previewUrl = cat.thumbnail_url || null;
            this.showModal = true;
        }
    }">
        ...
        <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-6">
            <div>
                ...
                <h1 class="text-2xl lg:text-3xl font-bold text-slate-900 tracking-tight">Category Master</h1>
                <p class="text-slate-500 text-sm mt-1">Manage main product categories, display sequence and catalog thumbnails.</p>
            </div>
            <div>
        ...
        <div class="bg-white p-4 rounded-2xl border border-slate-200/80 shadow-sm mb-6">
            <form method="GET" action="{{ route('categories.index') }}" class="flex flex-col sm:flex-row items-center justify-between gap-4">
                <div class="relative flex-1 w-full max-w-xl">
                    ...
                    <input type="text" name="q" value="{{ $q }}" placeholder="Search category name..." class="w-full pl-10 pr-4 py-2 bg-slate-50 border border-slate-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-500 transition">
                </div>
                            ...
                            <tr class="hover:bg-slate-50/50 transition">
                                ...
                                <td class="px-6 py-4">
                                    @if($category->thumbnail_url)
                                        <img src="{{ $category->thumbnail_url }}" alt="{{ $category->name }}" class="w-11 h-11 rounded-xl object-cover border border-slate-200 shadow-sm">
                                    @else
                                ...
                                <td class="px-6 py-4">
                                    <div class="font-bold text-slate-900 text-base">{{ $category->name }}</div>
                                    <div class="text-slate-400 text-xs font-mono mt-0.5">{{ $category->slug }}</div>
                                </td>
                                ...
                                <td class="px-6 py-4">
                                    <a href="{{ route('sub-categories.index', ['category_id' => $category->id]) }}" class="inline-flex items-center gap-1 text-emerald-600 hover:text-emerald-700 font-semibold text-xs bg-emerald-50 px-3 py-1.5 rounded-full hover:bg-emerald-100 transition">
                                        <span>{{ $category->sub_categories_count ?? $category->subCategories()->count() }} Sub-Categories</span>
                                        <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M14 5l7 7m0 0l-7 7m7-7H3"></path></svg>
                                    </a>
                                </td>
                                <td class="px-6 py-4 font-medium text-slate-800">
                                    <a href="{{ route('products.index', ['category_id' => $category->id]) }}" class="hover:text-emerald-600 transition">
                                        {{ $category->products_count ?? $category->products()->count() }} Products
                                    </a>
                                </td>
                                <td class="px-6 py-4">
                                    <form method="POST" action="{{ route('categories.toggle', $category) }}" class="inline-block">
                                        @csrf
                                        <button type="submit" class="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-semibold transition cursor-pointer {{ $category->status ? 'bg-emerald-100/90 text-emerald-800 hover:bg-emerald-200' : 'bg-rose-100/90 text-rose-800 hover:bg-rose-200' }}">
                                ...
                                <td class="px-6 py-4 text-right">
                                    <div class="flex items-center justify-end space-x-2">
                                        <button @click='openEditModal(@json($category))' class="p-2 text-slate-500 hover:text-emerald-600 hover:bg-emerald-50 rounded-lg transition" title="Edit Category">
                                        ...
                                        <form method="POST" action="{{ route('categories.destroy', $category) }}" onsubmit="return confirm('Are you sure you want to delete this category?')" class="inline-block">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit" class="p-2 text-slate-500 hover:text-rose-600 hover:bg-rose-50 rounded-lg transition" title="Delete Category">
        ...
        <div x-show="showModal" class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm" x-cloak x-transition>
            <div @click.outside="showModal = false" class="bg-white rounded-2xl shadow-2xl max-w-lg w-full p-6 border border-slate-100 relative">
                ...
                <form :action="actionUrl" method="POST" enctype="multipart/form-data" class="space-y-4">
                    ...
                    <div>
                        <label class="block text-xs font-semibold text-slate-700 uppercase mb-1">Category Name <span class="text-rose-500">*</span></label>
                        <input type="text" name="name" x-model="name" required placeholder="e.g. Kids Wear" class="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-500 transition">
                    </div>
                    ...
                    <div>
                        <label class="block text-xs font-semibold text-slate-700 uppercase mb-1">Category Thumbnail</label>
